...
async def stream_gemini_response(
    ...
    elif model_name == Llm.GEMMA_3_27B.value:
         api_model_name = "gemma-3-27b"

    print(f"DEBUG: model_name='{model_name}' expected='{Llm.GEMMA_3_27B.value}'")

    if model_name == Llm.GEMMA_3_27B.value:
         # Gemma 3 does not support system_instruction, so we prepend it to the first user message
         if gemini_contents and len(gemini_contents) > 0:
             # Ensure the first content has parts
             if not gemini_contents[0].parts:
                  gemini_contents[0].parts = []
             
             # Create a new text part with the system prompt
             system_part = types.Part(text=f"Instructions: {system_prompt}\n\n")
             # Insert it at the beginning of the first message's parts
             gemini_contents[0].parts.insert(0, system_part)
         
         # Initialize config as a dict to ensure no hidden system_instruction field is sent
         config = {
             "temperature": 0.0,
             "max_output_tokens": 8000
         }
    elif model_name == Llm.GEMINI_2_5_FLASH_PREVIEW_05_20.value:
