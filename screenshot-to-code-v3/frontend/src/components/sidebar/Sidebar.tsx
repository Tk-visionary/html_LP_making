...
function Sidebar({
  ...
  const { pendingEdits, clearPendingEdits, removePendingEdit } = useAppStore();

  const handleBatchUpdate = () => {
    if (pendingEdits.length === 0) return;

    const consolidatedPrompt = `Apply the following ${pendingEdits.length} changes to the code. Ensure each change is applied correctly to the specified element context.\n\n` + 
      pendingEdits.map((edit, i) => 
        `CHANGE #${i + 1}:\nTARGET CONTEXT:\n${edit.element.outerHTML}\n\nINSTRUCTION:\n${edit.instruction}`
      ).join("\n\n--------------------------------------------------\n\n");

    doUpdate(consolidatedPrompt, includeThoughts);
    clearPendingEdits();
  };
          ...
          >
            <div className="grid w-full gap-2 relative">
              ...
              {pendingEdits.length > 0 && (
                <div className="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-md p-3 mb-2">
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto mb-3">
                    {pendingEdits.map((edit, idx) => (
                      <div key={idx} className="text-xs bg-white dark:bg-gray-800 p-2 rounded border border-gray-200 dark:border-gray-700 flex justify-between gap-2">
                        <div className="truncate flex-1 font-mono text-gray-600 dark:text-gray-400">
                          {edit.instruction}
                        </div>
                        <button 
                          onClick={() => removePendingEdit(idx)}
