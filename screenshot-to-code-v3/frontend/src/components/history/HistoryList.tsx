...
const HistoryList: React.FC<Props> = ({ onSelect }) => {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${HTTP_BACKEND_URL}/history/list`);
      if (res.ok) {
  ...
  const handleSelect = async (item: HistoryItem) => {
    try {
      const res = await fetch(`${HTTP_BACKEND_URL}${item.url}`);
      if (res.ok) {
        const data = await res.json();
        onSelect(data);
      }
  ...
  const handleDelete = async (e: React.MouseEvent, item: HistoryItem) => {
    ...
    try {
      const res = await fetch(`${HTTP_BACKEND_URL}/history/${item.date}/${item.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  const handleSaveTitle = async (e: React.SyntheticEvent, item: HistoryItem) => {
    ...
    try {
      const res = await fetch(`${HTTP_BACKEND_URL}/history/title`, {
        method: "POST",
