...
function App() {
  ...
  const [settings, setSettings] = usePersistedState<Settings>(
    {
      ...
      codeGenerationModel: CodeGenerationModel.GEMINI_2_5_FLASH,
  ...
  function doCreate(
    ...
    if (referenceImages.length > 0) {
      doGenerateCode({
        ...
        inputMode,
        prompt: { text: textPrompt, images: referenceImages },
        isTemplateEnabled: settings.isTemplateEnabled,
        templateContent: settings.templateContent,
        // @ts-ignore
        includeThoughts: false, // Initial create doesn't have thoughts
  ...
  function restoreHistory(session: any) {
    ...
    const historyStack = session.stack || [session];
    let parentHash: string | null = null;
    let lastCommit: Commit | null = null;

    historyStack.forEach((historyItem: any, index: number) => {
        // Restore params from the last item (or merged params?)
        // Usually the last item dictates the current settings
        if (index === historyStack.length - 1 && historyItem.params) {
              if (historyItem.params.input_mode === "text") {
                setInputMode("text");
                setInitialPrompt(historyItem.params.prompt?.text || "");
              } else {
                setInputMode(historyItem.params.input_mode || "image");
              }
              
              if (historyItem.params.generation_type === "create" && historyItem.params.stack) {
                 setSettings((s) => ({
                    ...s,
                    generatedCodeConfig: historyItem.params.stack,
                 }));
              }
        }

        const variants = (historyItem.variants || []).map((v: any) => ({
          code: v.code || "",
        }));
        ...
        const commit = createCommit({
          type: index === 0 ? "ai_create" : "ai_edit", 
          parentHash: parentHash,
          variants: variants,
          inputs: {
             text: historyItem.params?.prompt?.text || "",
             images: [], // Still TODO
          },
        });
    ...
    if (lastCommit) {
        setHead((lastCommit as Commit).hash);
    }
    ...
    <div className="mt-2 dark:bg-black dark:text-white">
      ...
      <div className="lg:fixed lg:inset-y-0 lg:z-40 lg:flex lg:w-96 lg:flex-col">
        <div className="flex grow flex-col gap-y-2 overflow-y-auto border-r border-gray-200 bg-white px-6 dark:bg-zinc-950 dark:text-white">
          ...
          {IS_RUNNING_ON_CLOUD && !settings.geminiApiKey && <OnboardingNote />}

          {appState === AppState.INITIAL && (
            <>
              <GenerateFromText doCreateFromText={doCreateFromText} />
              <div className="mt-4">
                 <HistoryList onSelect={restoreHistory} />
              </div>
            </>
          )}
          ...
          {(appState === AppState.CODING ||
            ...
            <Sidebar
              ...
              doUpdate={(instruction, includeThoughts) =>
                doUpdate(instruction, undefined, includeThoughts)
              }
      ...
      <main className="py-2 lg:pl-96">
        ...
        {(appState === AppState.CODING || appState === AppState.CODE_READY) && (
          <PreviewPane reset={reset} settings={settings} />
        )}
      </main>
    </div>
  );
