...
このリポジトリでは、ランディングページ（LP）を効率的に作成する**2つの方法**を検証しています。

## 🔬 検証中の2つのアプローチ

| 方式                                   | 説明                                                                                 | ディレクトリ             |
| :------------------------------------- | :----------------------------------------------------------------------------------- | :----------------------- |
| **方式1: テンプレート生成**            | 企画書（JSON）とデザインスタイルを選択するだけで、Jinja2テンプレートからLPを自動生成 | ルート (`generator.py`)  |
| **方式2: スクリーンショート → コード** | スクリーンショットをAIに渡し、HTMLコードを自動生成。GUIで編集も可能                  | `screenshot-to-code-v3/` |

---

# 方式1: テンプレート生成 (generator.py)
...
# 方式2: スクリーンショート → コード (screenshot-to-code-v3)

スクリーンショット、モックアップ、Figmaデザインを、AIを使ってクリーンで機能的なHTMLコードに変換するGUIツールです。

## 特徴

- **バッチ編集モード**: 複数の修正指示を一括適用
- **分割＆結合生成**: 長いLPを複数画像でアップロード → 各セクションを個別コーディング → 1ファイルに結合
- **色精度の向上**: テキスト色と背景色の組み合わせを厳密に再現
- **画像自動トリミング**: AIが座標を指定 → 元スクリーンショットから該当領域を切り出し → ファイル保存
- **背景自動透過 (rembg)**: 人物写真は `u2net_human_seg`、イラスト等は `isnet-general-use` で背景削除
- **コード内編集 & 保存**: GUI上でコードを編集し、保存ボタンでプレビューに反映
- **テレメトリなし**: データ追跡なし

## セットアップ

```bash
cd screenshot-to-code-v3/backend
...
```bash
cd screenshot-to-code-v3/frontend
yarn
yarn dev
```
...
1. スクリーンショットをドラッグ＆ドロップ
2. スタック選択（HTML/Tailwind推奨）
3. AIモデル選択（Gemini 3 Flash推奨）
4. 「Generate Code」をクリック
5. プレビュー確認 & 編集

詳細は `screenshot-to-code-v3/README.md` を参照してください。
